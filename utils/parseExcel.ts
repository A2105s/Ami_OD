...
function extractEventMetadata(data: string[][]): EventMetadata {
  ...
  for (let i = 0; i < Math.min(12, data.length); ++i) {
    ...
    // Prefer primary value at col 1; if empty, consider alt at col 3 (index 3) when present
    // Ignore when the adjacent header (col 2) suggests example fields
    let value = String(row[1] ?? '').trim();
    const col2Header = String(row[2] ?? '').toLowerCase();
    const altValue = String(row[3] ?? '').trim();
    if (!value && altValue && !/example/.test(col2Header)) {
      value = altValue;
    }
    if (!value) continue;
    ...
    for (const [fuzzy, key] of (Object.entries(metaKeys) as Array<[string, keyof EventMetadata]>)) {
  ...
  // Error if none found
  const allMetaFields: Array<keyof EventMetadata> = [
    'eventName','eventDate','eventTime','eventVenue','coordinator','day','place'
  ];
  const missing = allMetaFields.filter((k) => !(meta[k] as string));
...
function extractStudents(rows: any[][]): Student[] {
  ...
  for (let i = headerRowIndex + 1; i < rows.length; i++) {
    ...
    try {
      ...
      const name = toTitleCase(cleanStudentName(nameRaw));
...
/**
 * Removes serial prefixes and extraneous markers from student names.
 * Examples: "1) Adarsh Singh" -> "Adarsh Singh", "02. namma" -> "namma".
 */
function cleanStudentName(name: string): string {
  const s = String(name || '').trim();
  // Remove leading serials like "1)", "1.", "01.", "1 -", "1:"
  const withoutSerial = s.replace(/^\s*\d+\s*[).\-:]?\s*/i, '');
  // Collapse spaces
  return withoutSerial.replace(/\s+/g, ' ').trim();
}
